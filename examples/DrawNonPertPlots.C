#include "TObject.h"
void DrawNonPertPlots(TString ALGO)
{
  TFile *f;
  TString ss(ALGO+"canvasOutFile.root");
  f = new TFile(ss);
  TCanvas *c;
  c = (TCanvas*)f->Get("_R48094");
  ...
  gPad->SetLogx();
  histList[0]->SetMinimum(0.95);
  histList[0]->SetMaximum(1.3);
  histList[0]->SetTitle("");
  ...
  histList[0]->GetYaxis()->SetTitle("Non-perturbative correction");
  ...
  histList[0]->Draw("E1");
  histList[1]->Draw("sameE1");
  histList[2]->Draw("sameE1");
  ...
  leg->SetTextFont(42);
  leg->AddEntry(histList[0],"0.00 #leq |y| < 0.55","LP");
  leg->AddEntry(histList[1],"1.10 < |y| < 1.70","LP");
  leg->AddEntry(histList[2],"1.70 < |y| < 2.50","LP");
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  leg->Draw();

  TPaveText *pave3 = new TPaveText(0.47,0.55,0.92,0.7,"NDC");
  ...
  c->Print(ALGO+"_NonPerturbative.eps");
