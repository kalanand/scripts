...
// Lower end point shape: second Gaussian at 0, different width
  ...
  // Upper end pointGaussian: a narrow Gaussian at 5
  RooGaussian gaus2("gaus2","gaus2",x, RooFit::RooConst(5),RooFit::RooConst(1.0) ) ;
  // Create interpolation variable between gaus1 and gaus2
  RooRealVar gamma("gamma","gamma", 0.8, 0., 1.) ;
  gamma.setBins(10,"cache") ;

  // Upper end point shape: template morphing between gaus1 and gaus2
  RooIntegralMorph g2("g2","Upper end point shape",gaus1, gaus2,x,gamma) ;
  g2.setCacheAlpha(kTRUE) ;
